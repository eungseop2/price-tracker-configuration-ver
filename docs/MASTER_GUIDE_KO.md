...
### [AI 마법 프롬프트 복사하기]
```text
너는 '네이버 쇼핑 최저가 트래커'의 설정 전문가야. 내가 아래에 주는 상품 정보를 바탕으로 프로그램 규격에 맞는 `targets.yaml` 파일 내용을 생성해줘.

### 📋 내가 추적하고 싶은 정보
1. 상품명: [여기에 대시보드에 표시할 이름 입력, 예: 갤럭시 S26 울트라]
2. 검색어: [여기에 네이버 쇼핑 실제 검색어 입력]
3. 카탈로그 ID: [여기에 네이버 쇼핑 상세페이지 URL 끝의 숫자 입력]
4. 추적 셀러: [여기에 감시할 업체명들을 쉼표로 구분해서 입력]
5. 순위 키워드: [여기에 노출 순위를 확인할 키워드들을 입력]

### 🛠️ 출력 규칙 (필수 준수)
- 반드시 YAML 코드 블록 형식으로만 출력할 것.
- `items` 리스트에는 `name`, `query`, `product_id`를 포함할 것.
- `monitored_sellers`와 `rank_query` 섹션은 리스트 형태로 상단에 배치할 것.
- product_id는 반드시 따옴표('')로 감싸서(문자열로) 출력할 것.
- 불필요한 설명은 생략하고 오직 코드만 출력할 것.
```
