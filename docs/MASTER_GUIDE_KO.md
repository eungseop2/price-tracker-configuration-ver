...
> **내 입력 예시:**
> "아이템은 'S26 울트라 자급제', 검색어는 '갤럭시 S26 울트라 자급제폰', 카탈로그 ID는 '1234567890'이야. 내가 감시하고 싶은 셀러는 '위드모바일'이랑 '삼성디지털프라자'고, '자급제 추천' 키워드 순위도 보고 싶어. 카테고리는 '가전'으로 해줘."
...
- **기능**: 상품명을 클릭하면 우측에 **실시간 가격 그래프**와 **상세 히스토리 테이블**이 즉시 나타납니다!

> [!TIP]
> **계층 요약**: `가전(Category)` > `위드모바일(Mall)` > `S26 울트라 512GB(Product)`  
