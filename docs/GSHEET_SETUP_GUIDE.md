# 📊 구글 스프레드시트(GSheet) DB 연동 상세 가이드 (DB-test)
...
2.  **API 및 서비스 > 라이브러리**에서 `Google Sheets API`와 `Google Drive API`를 각각 **[사용]**으로 변경합니다.
...
5.  다운로드된 `.json` 파일을 메모장으로 열어 내용(`{ }` 포함 전체)을 복사해 둡니다. (이것이 `GOOGLE_SERVICE_ACCOUNT_KEY` 또는 `GCP_SA_KEY`가 됩니다.)
...
3.  **[공유]** 버튼 클릭 > 2단계에서 만든 서비스 계정 이메일(예: `...-sa@...iam.gserviceaccount.com`)을 추가합니다.
